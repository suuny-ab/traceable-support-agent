...
> 当前版本是公开 Beta 真实 Provider live 版（`2026-07-29` 上线，用户显式授权），健康状态为 `available`，生成门为绑定式溯源（ADR-0007），回放预览保持可用。Stage 12 已执行一次（19/24 案例、9 通过），暴露候选生成合同高失败率和两条边界缺陷；`product/0.1.0` 尚未发布。当前代码已为公开合成安全风险和 CZ-R1 / CZ-R2 独占能力冲突增加生成前确定性转人工边界，但没有重跑或改写 Stage 12。
