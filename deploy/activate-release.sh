...
if test -L "$release_root/current"; then
  ...
  if test "$old_release" = "$release_dir"; then
    release_wait_ready "$release_dir" "$public_origin" "$(release_live_enabled "$release_dir")" "$candidate_sha"
...
restore_old() {
  ...
  if test -n "$old_release"; then
    ...
    release_wait_ready "$old_release" "$(release_public_origin "$old_release")" "$(release_live_enabled "$old_release")" "$old_sha"
  fi
}
...
if ! release_wait_ready "$release_dir" "$public_origin" "$(release_live_enabled "$release_dir")" "$candidate_sha"; then
  restore_old
