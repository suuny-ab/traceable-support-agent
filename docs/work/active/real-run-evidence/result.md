...
> 状态：候选已推送并为 Draft PR #37（base `main`）；冻结复核基线 `b30221a`
> 四项 Checks 全绿后经一次正式独立复核，两项阻断成立（合同不能忠实表达
> transport 未知账单与部分失败事实；候选事实文档滞后于 PR 生命周期）。已完成
> 合同忠实性修复与文档同步，全离线本地验证通过；新 head 四项 Checks 全绿后
> 仅对原 finding 与覆盖 diff 定向复核。
...
- 阻断修复覆盖 diff：`contract.py`、`test_real_run_transcript.py`、本工作记录与
  ...
  公开合同 / manifest。
- 第 2 轮（待执行）：新 head 四项 Checks 全绿后，仅对两项原 finding 与覆盖
  diff 定向复核一次。
