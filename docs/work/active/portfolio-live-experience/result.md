...
Worker 主导，而不默认由 Codex 验收。当前已启动最小多 CLI 协作实验：首页 v4 是产品
载体，Codex 是唯一用户入口，CLI 状态由 Codex 报告、实际 Worker 由用户选择。
...
- 用户只与 Codex 交互；Codex 不自动选择开发 CLI，只报告可用性、模型、额度可见性和
  当前任务，等待用户选择；
- 初版不开发调度程序、自动路由或额度评分，不安装或调用 Qoder；
- Codex 负责任务合同、写入权、worktree、进度和集成；专业视觉判断归具备视频能力的
  Worker，最终体验归用户；
- 一个 worktree 只允许一个写入者；需要并行写入时使用独立 worktree；
- 工作流只有在真实产品结果中降低等待或重复工作且协调成本可接受时才允许保留。
...
这些检查只证明当前 checkpoint 可恢复、原型和对齐稿能在本机运行，不代表 v3 视觉通过或
多 CLI 工作流已经验证成功。
...
当前现场通过检查并形成 checkpoint commit。随后 Codex 输出首张 CLI 状态卡，由用户明确
选择首页 v4 的开发 Worker；选择前不启动新的外部 CLI 写入任务。第二屏继续作为独立设计
问题，不沿用 v3。
