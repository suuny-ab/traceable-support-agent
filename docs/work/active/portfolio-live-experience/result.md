...
Worker 主导，而不默认由 Codex 验收。多 CLI 协作已由用户直接决定为正式元开发规则，
但首页 v4 不再被预设为首个任务；下一任务、范围和 Worker 留给新会话重新确认。
...
- 唯一详细规则为 `docs/meta/multi-cli-workflow.md`，`AGENTS.md`只保存摘要和启动路由；
- 用户只与 Codex 交互；没有开发任务时 Codex 作为项目规划者，双方形成开发决定后才报告
  CLI 可用性、模型和当前额度，由用户选择 Worker；
- 一个任务默认使用一个有边界的多轮 Worker 会话；一个 worktree 同时只有一个写入者；
- 不设置任务卡；Plan 模式只辅助讨论，Worker 自己维护 `spec / plan / result` 和执行期状态，
  独立复核者维护 `review`；
- Codex 使用自然协作式提示，逐字公开发给 Worker 的每条消息，并摘要转述 Worker 的每次
  回复；共享 `AGENTS.md` 定义仓库规则，派发消息定义当前身份；
- 初版不开发调度程序、自动路由或额度评分；Qoder 未安装或核实前不得报告为可用；
- Kimi Code 当前额度已证明可以通过本机只读接口核实，但具体数值易变，不进入长期文档；
- 本规则由用户直接采用，不宣称已经通过效率实验；后续遇到具体问题时按证据修订。
...
并逐项解释 v3 的方向、形变、扫描线、月牙与手机缩放差距。这证明 K3 能承担动态视觉分析，
也证明其长步骤运行期间存在可被 Codex 利用的并行时间。
...
完成正式规则最终修订和 checkpoint commit。新会话由 Codex 作为项目规划者陪同讨论；双方
形成开发决定后才生成新的 CLI 状态卡，由用户选择 Worker 并透明派发。首个任务不预设为
首页 v4。
