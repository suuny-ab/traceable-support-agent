...
- 用户只与 Codex 交互；范围确认后 Codex 报告可用性、模型和当前额度，用户选择 Worker；
- 一个任务默认使用一个有边界的多轮 Worker 会话；一个 worktree 同时只有一个写入者；
- Plan 模式只负责收敛，`spec / plan / result / review`分别保存范围、执行、结果和复核；
...
完成正式规则持久化、重复文档清理和 checkpoint commit。新会话先重新确定任务范围；范围
确认后 Codex 才生成新的 CLI 状态卡，由用户选择 Worker。首个任务不预设为首页 v4。
