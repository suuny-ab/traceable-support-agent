...
1. 把已经确认的 Codex 规划角色、Plan 模式、CLI 状态、用户选择、自然派发、Worker 会话、
   过程文档所有权、worktree、双向消息透明、回执和分层验收写入
   `docs/meta/multi-cli-workflow.md`，并由 `AGENTS.md` 路由。
...
6. 新会话由 Codex 作为项目规划者陪同用户讨论；形成开发决定后才生成新的 CLI 状态卡，
   由用户选择 Worker。Codex 逐字公开出站消息、摘要转述每次入站回复；旧会话额度、授权
   和 Worker 选择不继承。
