...
- 候选 SHA：`dbd2941f104fef2e6c5ad55523baa6b4a98142ec`。
- 前置证据：CI run `30060109472` 的四项必需 Checks 全绿；工作树干净且候选固定。
- 已关闭：复核指定的“是否支持开启……”与“……可以设置吗”能力问法放行。
- 结论：`failed`；该 SHA 不得合入。
- 剩余阻断：宽泛能力模式会把裸 `E310` 或“集尘袋已满”与重置动作的组合误判为能力
  问答，使明确故障 / 已满操作请求进入 Runner。
