...
- 第一阶段提高到 `16384` 后，TK-001 是否不再触发 `finish_reason=length`。
- 三次 R1 尝试中未获得 usage 的调用实际账单。
...
未放松已声明硬门；真实 API 已证明 QA-003 可以形成完整候选，并把 TK-001 的第一阶段
失败定位为当前 `8192` 输出上限导致的 `length`。尚不能声称 `16384` 已在真实 API
修复、全部合同兼容、成功率改善、生产就绪、发布或用户验收。
