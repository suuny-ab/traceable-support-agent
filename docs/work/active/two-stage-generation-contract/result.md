...
- 第一阶段改为 LLM 输出义务自然语言描述与语义支撑的 `clause_ids`；宿主仍验证 clause
  身份、完整互斥分区和大小上限，并派生 `evidence_ids` 与忽略上下文。
- 生产合同不再要求、验证或向第二阶段下传 `key_elements`；历史 v2 兼容路径未改变。
- 第二阶段继续由 LLM 声明来源跨度与客户可见语义跨度；宿主的来源存在、客户跨度存在、
  义务集合、安全、型号、隐私、预算、Provider 信封与人工批准硬门未放松。
- 代码：
  `cece633a6b11af3f36a116ca3a793b0f8654b94c`；镜像：
  `sha256:a8cfbede1481a38a2eb97dcf0abcbc36e9ca210de01af7cc378574db90fe4bb8`，
  revision 精确匹配，运行用户 `10001:10001`。
- 定向合同 / 产品 / 探针 53 项通过；Fast 公开扫描 `187` 个文件 / 8 个公开案例，
  工具 `78` 项通过（7 项环境门跳过）；完整 API Candidate `96` 项通过；Web lint、
  typecheck、生产构建和 18 项协议 / 页面测试通过。
- live 镜像在无网络条件下完成 8/8 公开检索案例，Provider 调用 `0`。
