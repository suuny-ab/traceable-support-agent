...
- v4 的义务数量失败属于类型、空列表还是超过 8 项。
- 四次 R1 尝试中未获得 usage 的调用实际账单。
...
未放松已声明硬门；真实 API 已证明 QA-003 可以形成完整候选，且 `16384` 消除了本次
TK-001 第一阶段的 `length`。当前阻碍转为 checklist 义务数量合同；尚不能声称工单
候选可用、全部合同兼容、成功率改善、生产就绪、发布或用户验收。
