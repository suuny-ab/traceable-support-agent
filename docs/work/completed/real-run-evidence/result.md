...
> 状态：候选为 Draft PR #37（base `main`）。head 序列：合同修复 `9c7ed59` →
> 第 1 轮文档同步 `b49e379`（run 30421455186 四项 Checks 绿，第 2 轮复核对象）
> → 事实收口 `f5a0971`（run 30422565893 绿）→ 本轮 plan.md 旧约束与自指引用
> 修正；产品代码自 `9c7ed59` 起未变，各推送 head 四项 Checks 均绿，最新 head
> 与 Checks 以 PR #37 为准。第 1 轮正式复核两项阻断（合同不能忠实表达
> transport 未知账单与部分失败事实；候选事实文档滞后于 PR 生命周期）已完成
> 修复；第 2 轮定向复核确认代码语义修复通过，结论 `BLOCKED`：唯一剩余阻断为
> PR 说明与项目事实停在旧候选，`f5a0971` 与本轮修正即该阻断的收口。
...
- 收口后自查（本轮修正）：`plan.md` 残留旧计划约束（real
  `network_attempt_count>=1`），与已评审合同（credential_missing 忠实零
  network、network 计数不设下界）冲突，已改为实际语义；状态材料中"当前
  head / CI"的自指引用（`b49e379` / run 30421455186）改为带时间锚点的历史
  表述；PR 说明中"transcript 与 transport observations 持久化"的宽表述
  修正为仅持久化 `provider_observations`。产品代码不变。
