...
达到首屏预期；Moonshot 首屏的可测量参考对齐已经形成；用户进一步确认动态网页应由具备
视频视觉能力的 Worker 主导，而不默认由个人 Work 前台验收。当时开发协作按项目工程规则
执行。

历史阶段中的生产边界：全程保持 `replay_only`，未启用产品 Provider，未读取产品凭据，未
修改生产开关或部署状态。Kimi Code CLI 的公开参考视觉分析来自当时会话中的 Git 外明确
授权，不构成产品 Provider 授权，也不由本文件延续授权。
...
针对性复核确认候选行为与四项 checks 无问题，剩余三处事实一致性残余，本轮最小修正：

1. `spec.md` 前端设计合同段的旧概括（"实时不可用时只明确说明不能创建新的运行"）改为
   统一合同表述（普通运行不创建 + 固定边界挑战例外）；
2. `docs/status.md` 更新到当前唯一阶段：候选冻结 `f873906` 后等待针对性复核；
3. PR #31 描述重写为单一事实入口（只保留当前候选、当前验证口径 24/24、复核历史）。
4. 验收保护：`boundary challenge copy declares the live-off exception honestly` 增加对
   `spec.md` 的两处旧概括原文禁回归断言。本机 `npm test` 24/24 通过，公开扫描 passed。

---

## 最终结果：求职交付部署与生产体验验收（2026-07-28）

### 交付与验收证据

- PR #31（live 优先工作台 + 统一公开运行合同）与 PR #34（首页预览绑定 `KB-CZR1-014`）
  已 squash 合并；当前 `main` 为 `34079d7`；
- main CI（run 30324344653）五项全绿，镜像按摘要发布；生产部署（run 30324478567）
  preflight + deploy 成功并绑定 `34079d7`；
- 公开环境 <https://47.84.34.86/>：健康接口 `status=ok`、`live_experience=replay_only`；
  首页与工作台的局部清扫内容统一为"长按三秒"并绑定 `KB-CZR1-014`；
- Codex 独立最终生产体验验收：`PASS`。

### 范围决定（用户）

- 当前求职交付 = 已部署并通过最终体验验收的公开 Beta 回放版；生产继续 `replay_only`，
  真实 Provider、凭据和费用均未启用；`product/0.1.0` 仍未发布；
- Issue #28 原始范围中的公开真实 Provider 与实时 candidate 验收门**没有完成**，用户已
  决定不纳入当前求职收尾；本记录不声称该 Issue 原验收门全部通过；
- Issue #29 保留为下一项 GitHub 求职展示工作；Issue #14 继续后置；
- 当前没有产品代码修复阻断；在状态事实收口候选通过独立复核并进入 main 前，不宣布整个
  项目正式收口。

### 拟议的 Issue #28 关闭说明（建议原因 `not planned`）

> 本 Issue 的作品集视觉、公开回放体验与生产部署验收已经完成：live 优先工作台、统一
> 公开运行合同、首页预览与批准来源 `KB-CZR1-014` 一致性均已上线，并通过独立最终生产
> 体验验收。原始范围中的"公开真实 Provider 受控真实体验"目标，按当前求职收尾的取舍
> 明确停止，不作为已完成申报；生产保持 `replay_only`。保留全部历史记录，以
> `not planned` 关闭。

### 生命周期语义修正（2026-07-28，复核第二轮）

复核指出三处事实阻断，本轮修正：

1. `PROJECT.md` 同步为当前结果：视觉与回放体验已完成并通过验收，公开真实 Provider 目标
   已停止且从未启用，`replay_only` / S1 Beta / `0.1.0` 未发布表述与各权威入口一致；
2. Issue #28 生命周期语义统一：`ROADMAP.md` 不再把整个 Issue 放入普通“已完成”，
   `spec.md` 状态回到 `developing` 并明确“Issue 仍为 OPEN，待按 `not planned` 关闭”，
   旧视觉阶段表述已历史化；拟议关闭说明保留在上一节；
3. `docs/status.md` 改为合并前后均成立的表述：区分“当前已部署产品版本 `34079d7`”与
   “仓库当前基线（`origin/main` 当前 head，不固定 SHA）”，移除合并后立即失真的
   “候选待复核 / candidate_ready”表述。
