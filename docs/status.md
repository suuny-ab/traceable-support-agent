...
| 当前治理候选 | Draft PR [#56](https://github.com/suuny-ab/traceable-support-agent/pull/56)，分支 `codex/status-two-layer`；head 与 Checks 以 GitHub 实时状态为准，保持 Draft、未合并 |
...
| Task | 状态 | 候选 / 下一动作 |
| --- | --- | --- |
| `status-two-layer` | `draft_pr` | PR #56；本地 Fast / 治理检查全绿；同一 head 的 required Checks 与用户合并授权缺一不可 |
...
只有 PR #56 同一 head 的 required Checks 全绿且取得用户合并 / 自动部署链授权后才可继续；本切片不转 Ready、不合并、不部署。
