...
| 当前动作 | 自动排队和人工审批门已经生效；端口修复后的第二次批准运行又在连接服务器前发现 `DEPLOY_HOST` 含 UTF-8 BOM。主机 secret 已精确重设，正在把五项 SSH 输入统一收口到可信预检 |
| 阻碍 | 统一 SSH 预检仍须通过 PR CI、合并检查和新的人工批准；生产尚未切换，回滚演练和用户验收通过前不得关闭迁移或清理旧仓 |
| Provider | 已禁用；`provider_enabled=false`、`provider_calls=0`、`provider_cost_cny=0` |
| 下一检查点 | 让统一 SSH 预检修复通过 PR CI；合并后的新生产候选必须再次停在人工批准页，再以固定 Git SHA 与镜像摘要执行“旧 → 新 → 旧 → 新”生产演练 |
