...
| 当前治理候选 | `codex/status-two-layer`；Draft PR 待创建 |
...
| Task | 状态 | 候选 / 下一动作 |
| --- | --- | --- |
| `status-two-layer` | `local_validated` | 本地 Fast / 治理检查全绿；推送并创建 Draft PR，不合并 |
...
推送 `codex/status-two-layer`、创建 Draft PR 并确认同一 head 的 Checks 启动；随后只等待，不合并、不部署。
