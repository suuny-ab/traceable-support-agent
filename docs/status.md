...
| 产品候选 | `codex/real-run-evidence` Draft PR #37：冻结复核基线 `b30221a`（四项 Checks 绿）正式复核两项阻断，合同忠实性修复（`9c7ed59`）与文档同步已完成；PR head 更新后待四项 Checks 与定向复核；当前已部署产品版本 `34079d7`（PR #31 / #34），仓库基线为 `origin/main` 当前 head |
...
| 下一检查点 | PR #37 修复 head 四项 Checks 全绿后，仅对两项原 finding 与覆盖 diff 做一次定向复核；合并、部署与真实 Provider 授权仍为独立用户决定；Issue #29（本地未推送展示分支保留）与 Issue #14 继续后置；定期依赖审计变红告警按分层处理 |
...
| `TASK-REAL-RUN-EVIDENCE` | `candidate` | Draft PR #37：冻结基线 `b30221a` 四项 Checks 绿；正式复核两项阻断（合同账单 / 失败事实不忠实、候选文档滞后）已修复并全离线验证，待新 head Checks 与定向复核；不启用实时模式，生产 `replay_only` 不变 |
