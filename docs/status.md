...
| 当前动作 | 自动排队和人工审批门已经生效；首次批准运行在连接服务器前因端口 secret 格式错误失败，secret 已重设为 `22`，正在修复端口校验的失败关闭 |
| 阻碍 | 端口校验修复仍须通过 PR CI 和合并检查；生产尚未切换，回滚演练和用户验收通过前不得关闭迁移或清理旧仓 |
| Provider | 已禁用；`provider_enabled=false`、`provider_calls=0`、`provider_cost_cny=0` |
| 下一检查点 | 让端口校验修复通过 PR CI；合并后的新生产候选必须再次停在人工批准页，再以固定 Git SHA 与镜像摘要执行“旧 → 新 → 旧 → 新”生产演练 |
