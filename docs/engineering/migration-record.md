...
这些证据关闭了迁移当时的公开远端、本机路径、生产部署、生产回滚演练、用户验收和全自动
部署检查点。上述 `replay_only` 是对应提交和回执的历史快照，不代表当前公网运行模式；
现行版本与 `live_experience` 以 `docs/status.md`、发布清单和公网健康接口为准。本记录仍不
形成 `product/0.1.0`、高可用或 SLA 主张。
