...
| Audit | 公开发布、历史边界或专项候选审计 | 按需 | 密钥/路径/大文件扫描、全新克隆和部署检查 |
...
公开主张与证据的核对遵循“可机器判断就进入 CI”。当前 `governance` 中的
`check_public_repo` 已覆盖仓库公开安全、关键状态词、固定回放与知识资产、部署 Workflow
结构；Web、API、容器和生产部署分别覆盖登记的页面、接口、健康、manifest 与回滚合同。
`ci_proof` 的绿色结果只证明实际运行且登记为通过的主张，`skipped` 或 `missing` 不构成
证据。

`check_public_repo` 目前不是所有公开文字的逐项事实核对器。新增或改变 README、公开页面
或其他公开文档中的机器可判定主张时，相关产品变更必须同时：

- 在 `docs/product/evidence-map.md` 绑定代码、测试、部署或运行证据；
- 扩展 `check_public_repo`、对应产品测试或部署 Workflow，从独立事实源核对该主张；
- 确保对应 Check 在该变更上实际执行，主张与证据漂移时失败关闭。

仅检查目标文案出现、人工阅读或 PR 中列出链接不算自动核对。缺少上述覆盖时，绿灯不能被
解释为该公开主张已由机器证明；需要动态生产状态的主张还必须等待绑定正确 SHA 的部署与
健康回执。
...
- `state` 只表示进度，不触发活动目录数量；轻量 / 标准工作可以没有活动目录。若为跨会话或
  隔离恢复创建目录，则该目录必须只含规范化四文件并由 `docs/status.md` 链接；
- 除仓库根目录外不存在嵌套 Git 仓库；
...
- 五个公开端点和稳定错误码；观测端点只返回聚合计数、延迟与错误分类，记录失败不改变主响应；
...
机器检查、当次授权和条件式只读评审兜底见 [`review.md`](review.md)。自动化覆盖不完整时
必须诚实标记边界，不能用默认独立复核或 CI 绿灯掩盖未知。
