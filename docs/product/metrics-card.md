...
- **数字**：原始正式观测计划 24 题、执行 19 题、9 题通过；调用 31/150、机制估算
  ...
  ¥1.2599124 / ¥10、自动重试 0，未提前停止。`2026-08-03` 今夜机械合同与 typed handoff
  候选后的唯一复跑为 24/24、11 题通过；调用 28/150、机制估算 ¥0.7169342 / ¥10、
  自动重试 0，未提前停止。
- **定义**：“执行 / 计划”是 runner 实际留下逐题聚合的数量；“通过”要求该题 outcome、
  来源、必需事实 / 义务及适用的工单字段全部满足。首次复验 22 题失败；今夜复跑 13 题失败，
  失败码出现次数为 14：字面事实缺失 6、义务规划缺失 6、outcome 与来源各 1。
- **测量方法**：三次均绑定候选、模型、prompt 哈希和同一私有集承诺，逐题运行真实
  Provider 并按当次冻结机械评分器聚合；两次复跑固定 24 案例 / 150 调用 / ¥10、整套一次、
  重试 0。评分合同不同的历史 package 重评分另列，不能与真实运行混写。
- **数据集**：24 题私有集只公开承诺哈希。原始聚合为
  ...
  [`stage12-night-fixes-revalidation-receipt-v1.json`](../../evals/stage12-night-fixes-revalidation-receipt-v1.json)；
  回执见相应活动工作结果。
...
- **今夜修复后唯一复跑**：六个预登记 typed handoff 全部命中、均 0 Provider 调用并满足
  ...
  14 码分别保留，不能拼成一个历史观测。28 次调用中 27 条有 usage，prompt 120,799、
  completion 94,377、total 215,176 tokens；1 次无有效 usage，机制估算不是账单确认。
- **单次 / 可复跑**：原始是全新未见正式观测；`2026-08-02` 与 `2026-08-03` 都使用已经
  消费的同一集，只是有界回归观测。该集今后仍只能回归；新正式结论要全新未见集、验证卡和授权。
- **边界 / 不是什么**：三次真实运行数字都不是线上成功率、“Stage 12 通过”或上线门。
  候选、评分合同、prompt 与执行覆盖不同，不能把前后差异归因给任何单一改动；历史结果不改写。
...
- **单次 / 可复跑**：0/2→3/3 是一次有授权的真实调用观察，不能无授权重跑；机械绑定合同
  可离线复跑。
- **边界 / 不是什么**：不是开放域准确率、长期 Provider 稳定性或 Stage 12 改善；最新
  已消费集回归为 11/24 全合同通过，四个 candidate 维度仍均为 0/3。候选仍由人工最终决定，
  绑定存在不等于结论正确。
