...
| QA 与工单产品路径支持两阶段生成 | 离线 `live` target 产品 fixture 与既有固定合成运行；v14 QA003 真实两阶段合同通过并形成 candidate；Stage 12 原始观测 19/24、9 通过（`evals/stage12-aggregate-v1.json`），修复后同一已消费集复验 24/24、2 通过（`evals/stage12-post-fix-revalidation-v1.json`）；PR #27 已合并并通过主线 CI/部署 | v14 仍缺公开回归必需事实；复验仍显示真实模型下来源、事实与 outcome 大量不匹配，候选质量主张不成立；复验不是新未见评测 |
| 每条客户可见结论绑定真实存在的来源和业务义务 | ADR-0007；QA / 工单合同测试覆盖措辞漂移放行、伪造 ID 拒绝、越源拒绝和义务缺绑定拒绝；本地真实 Provider 固定案例复测由 0/2 提升到 3/3，公网首条 QA 候选挂载证据原文 | 绑定存在不等于开放域语义正确；修复后 Stage 12 回归中来源章节不匹配 15/24，候选仍需人工审核 |
...
| 声明的公开合成安全事件与型号独占能力冲突可在生成前转人工 | `api/tests/test_product_boundaries.py`、公网 API 前置测试、`GEN-DEV-MH-001` 公开机械期望；transport factory 零构造、Provider 调用 0 | 只证明已声明规则及相邻负例；修复后 Stage 12 回归的安全 / 型号维度均为 0/3 全合同通过，也不证明开放域语义安全 |
| 候选 B 路线会把六类预登记跨型号、证据缺口与人工售后动作编译为 typed handoff | [`stage12-typed-handoff-boundaries/result.md`](../work/active/stage12-typed-handoff-boundaries/result.md)、产品边界 / 公共 API 测试、`GEN-DEV-MH-003` 公开机械期望；六个已消费私有案例只读结构回归均为 transport / Provider 调用 0 | 尚未合并或部署；只证明六个预登记案例与公开等价措辞，不是新的 Stage 12、开放域识别率、生成质量或发布主张 |
