...
| 可执行事实绑定来源和客户可见义务 | 精选公开回归；第一阶段 LLM 选择义务与 clause、宿主验证分区并派生 `approved_source_spans`，第二阶段 LLM 声明客户可见语义跨度、宿主机械校验来源跨度位于所选 clause、客户跨度存在和义务集合；v15 R0 攻击回归与冻结 head 针对性复核通过 | v14 真实回执早于 clause 级修复；v15 真实兼容性待验证；语义映射仍需要人工审核；Stage 12 多案例事实缺失 |
